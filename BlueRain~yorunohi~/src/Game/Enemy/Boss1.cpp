#include"Boss1.h"
#include<math.h>
#include"../../Lib/Common.h"
#include"../../Lib/MyMath/MyMath.h"
#include<cmath>

//#define MY_DEBUG 

static const int ROOT_ID[] = { 1,2,3,4,5 };
static const int ROOT_NUM = 5;


#define SHOTWAIT (1)
//コンストラクタ
Boss1::Boss1()
{
	memset(&m_Pos, 0, sizeof(m_Pos));
	memset(&m_Speed, 0, sizeof(m_Speed));

	m_isActive = false;
	m_hndl = -1;
	m_Radius = RADIUS;
	m_EnemyLife = ENEMYLIFE;



	m_Scale = VGet(10.0f, 10.0f, 10.0f);

	m_enemyKillcount = 0;

	m_count = 0;
	m_shotwait = 0;

	m_count = 0;
	m_speed = VGet(0.0f, 0.0f, 0.0f);

	m_waitflg = false;
	m_waitTimer = 10;
}

//デストラクタ
Boss1::~Boss1()
{
	Exit();
}

//初期化
void Boss1::Init()
{
	//memset(&m_Pos, 0, sizeof(m_Pos));
	memset(&m_Speed, 0, sizeof(m_Speed));

	m_isActive = true;
	m_hndl = -1;
	m_Radius = RADIUS;
	m_EnemyLife = ENEMYLIFE;


	m_Pos = VGet(0.0f, 200.0f, 0.0f);
	m_Scale = VGet(10.0f, 10.0f, 10.0f);

	m_enemyKillcount = 0;

	m_count = 0;
	m_Rand = GetRand(2);

	x = -1.7f;
	y = 0.0f;

	m_shotwait = 0;

	m_count = 0;

	m_speed = VGet(0.0f, 0.0f, 0.0f);

	m_waitflg = false;
	m_waitTimer = 10;

	m_Pos.x = WINDOW_SENTER_X - 300;
	m_Pos.y = WINDOW_SENTER_Y - 300;

	m_RequestCount = 200;

	m_RequestPostionState = 0;

	m_wait = 60;
}

//ロード
void Boss1::Load()
{
	if (m_hndl == -1)
	{
		m_hndl = LoadGraph("Data/GRED3976.png");
	}
}

//終了
void Boss1::Exit()
{
	if (m_hndl != -1)
	{
		DeleteGraph(m_hndl);
		m_hndl = -1;
	}
}


//描画
void Boss1::Draw()
{
	////生存フラグがオフの場合は終了
	//if (!m_isActive)
	//{
	//	return;
	//}
	if (m_isActive) {
		DrawCircle(m_Pos.x, m_Pos.y, 40, GetColor(0, 0, 255));
		DrawRotaGraph(m_Pos.x, m_Pos.y, 0.1f, 0.0f, m_hndl, TRUE);
	}
	DrawFormatString(20, 60, GetColor(255, 0, 0), "ボスの体力:%d", m_EnemyLife);
	//DrawFormatString(20, 20, GetColor(255, 0, 0), "弾が発射された数:%d", m_count);
#ifdef MY_DEBUG
	VECTOR Pos = m_Pos;
	//Pos.y = m_Radius;
	DrawSphere3D(Pos, m_Radius, 16, GetColor(0, 255, 255), GetColor(0, 0, 0), FALSE);
#endif
}

//敵の生成要請
bool Boss1::Request(const VECTOR& pos, const VECTOR& speed)
{
	//生存フラグがオンの場合は終了、エラーを外部に伝える
	if (m_isActive)
	{
		return false;
	}

	m_Pos = pos;
	m_Speed = speed;

	m_isActive = true;
	//出現成功を外部に伝える
	return true;
}


//毎フレーム
void Boss1::Step(ShotManager& shotmanager, Player& player)
{
	//生存フラグオフの場合は終了
	if (!m_isActive)
	{
		
	}


	
	//第一ウェーブ======================================
	if (m_EnemyLife > 2500) {//2501まではこの処理
		m_shotwait--;
		if (m_shotwait < 0) {
			shotmanager.RequestBossShot(m_Pos, m_Speed);
			m_shotwait = SHOTWAIT;
		}
	}
	//第2ウェーブ
	if (m_EnemyLife <= 2500 && m_EnemyLife > 1500)
	{
		m_wait--;
		if (m_wait < 0) {
			m_shotwait--;
			if (m_shotwait < 0) {
				shotmanager.RequestBossShot2(m_Pos, m_Speed);

				m_shotwait = SHOTWAIT;
			}
		}
	}
	if (m_EnemyLife <= 1500)
	{
		m_shotwait--;
		if (m_shotwait < 0) {
			shotmanager.RequestBossShot(m_Pos, m_Speed);
			shotmanager.RequestBossShot2(m_Pos, m_Speed);
			shotmanager.RequestBossShot3(m_Pos, m_Speed);

			m_shotwait = SHOTWAIT;
		}
	}
	//===================================================
	

	//移動制限
	if (m_Pos.x < -100 || m_Pos.x > 1030 ||
		m_Pos.y < 0 || m_Pos.y > 900)
	{
		m_isActive = false;
	}

}

bool Boss1::Hit(int Damage)
{
	m_EnemyLife -= Damage;
	if (m_EnemyLife <= 0) {
		m_isActive = false;

		//m_EnemyLife = ENEMYLIFE;


		return true;
	}
	return false;
}